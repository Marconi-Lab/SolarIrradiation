...
class ModisDataFetcher(ApiDataFetcher):
    ...
    def fetch_product_result(
        ...
        if response.status_code == 200:
            ...
            raise requests.exceptions.HTTPError(
                f"Failed to fetch data for product {product.name},coordinates {latitude},{
                    longitude} between dates: {start_date} and {end_date}: \n{response.text}"
            )
    ...
    def _validate_band_for_product(
        ...
        if band_name is None:
            if product.default_band is not None:
                ...
                raise ValueError(
                    f"No band provided for  product {
                        product.name}, and no default band available. "
            ...
            if not product.has_band(band_name):
                if product.default_band is not None:
                    logging.warning(
                        f"Band {band_name} not found in product {
                            product.name}, using default band {product.default_band.name} instead"
                    )
                    return product.default_band.name
                else:
                    raise ValueError(
    ...
    def get_available_dates_for_product_and_location(
        ...
        if response.status_code == 200:
            ...
            raise requests.exceptions.HTTPError(
                f"Failed to fetch available dates for product {
                    product.name} and coordinates {latitude}, {longitude}: \n{response.text}"
            )
