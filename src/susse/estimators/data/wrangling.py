...
def wrangle_CAMSRAD_data(path):
    """
    Process and clean CAMSRAD solar radiation data.

    Args:
        path (str): Path to the CAMSRAD CSV file.

    Returns:
        pandas.DataFrame: Processed dataframe with renamed columns and proper datetime formatting.
    """
...
def wrangle_NREL_Data(filepath):
    """
    Process and clean NREL solar radiation data.

    Args:
        filepath (str): Path to the NREL CSV file.

    Returns:
        pandas.DataFrame: Processed dataframe with combined datetime column and standardized units.
    """
...
def wrangle_Solcast_data(path):
    """
    Process and clean Solcast solar radiation data.

    Args:
        path (str): Path to the Solcast CSV file.

    Returns:
        pandas.DataFrame: Processed dataframe with selected columns and standardized units.
    """
    df = pd.read_csv(path)
    columns_to_keep = [
        "air_temp",
