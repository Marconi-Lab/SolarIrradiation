...
def test_mean_bias_deviation():
    ...
    assert StatisticalMetrics.mean_bias_deviation(obs, pred) == pytest.approx(-3.33, rel=1e-2)

def test_mean_absolute_deviation():
    obs = np.array([100, 200, 300])
    pred = np.array([110, 190, 290])
    assert StatisticalMetrics.mean_absolute_deviation(obs, pred) == pytest.approx(10, rel=1e-2)

def test_root_mean_square_deviation():
    obs = np.array([100, 200, 300])
    pred = np.array([110, 190, 290])
    assert StatisticalMetrics.root_mean_square_deviation(obs, pred) == pytest.approx(10, rel=1e-2)
...
def test_pearson_correlation():
    obs = np.array([100, 200, 300])
    pred = np.array([110, 190, 290])
    assert StatisticalMetrics.pearson_correlation(obs, pred) == pytest.approx(1, rel=1e-2)

def test_nash_sutcliffe_efficiency():
    obs = np.array([100, 200, 300])
    pred = np.array([110, 190, 290])
    assert StatisticalMetrics.nash_sutcliffe_efficiency(obs, pred) == pytest.approx(0.9, rel=1e-2)

def test_index_of_agreement():
    obs = np.array([100, 200, 300])
    pred = np.array([110, 190, 290])
    assert StatisticalMetrics.index_of_agreement(obs, pred) == pytest.approx(0.98, rel=1e-2)

def test_mean_absolute_percentage_error():
    obs = np.array([100, 200, 300])
    pred = np.array([110, 190, 290])
    assert StatisticalMetrics.mean_absolute_percentage_error(obs, pred) == pytest.approx(5, rel=1e-2)
