...
class TestRandomSplitter:
    ...
    def test_val_fraction_respected_within_one_row(
        ...
        train_idx, val_idx = RandomSplitter(val_fraction=0.25, random_state=0).split(
            toy_processed
        )
...
class TestTemporalSplitter:
    def test_rows_on_or_after_split_date_go_to_val(
        ...
        train_idx, val_idx = TemporalSplitter(split_date=date(2024, 2, 1)).split(
            toy_processed
        )
    ...
    def test_missing_date_column_raises_with_remediation(
        ...
        with pytest.raises(ValueError, match="FeatureSpec.id_columns"):
            TemporalSplitter(split_date=date(2024, 2, 1), date_column="when").split(
                toy_processed
            )
...
class TestStationLosoSplitter:
    ...
    def test_name_records_held_out_station(self) -> None:
        assert StationLOSOSplitter("sta_b").name == "station_loso[sta_b]"
...
class TestSpatialSpreadHoldoutSplitter:
    def test_n_holdout_two_picks_arctic_and_cape_town(
        ...
        train_idx, val_idx = SpatialSpreadHoldoutSplitter(n_holdout=2).split(
            geo_processed
        )
    ...
    def test_n_holdout_three_adds_the_farthest_third_station(
        ...
        _, val_idx = SpatialSpreadHoldoutSplitter(n_holdout=3).split(geo_processed)
        held = set(geo_processed.df.loc[val_idx, "location"].unique())
    ...
    def test_name_records_n_holdout(self) -> None:
        assert SpatialSpreadHoldoutSplitter(n_holdout=3).name == "spatial_spread[n=3]"
...
class TestSpatialBlockSplitter:
    def test_named_blocks_go_to_val(self, geo_processed: PreprocessedDataset) -> None:
    ...
    def test_missing_block_column_raises_with_remediation(
        ...
        with pytest.raises(ValueError, match="FeatureSpec.id_columns"):
            SpatialBlockSplitter(val_blocks=("uganda",), block_column="region").split(
                geo_processed
            )
